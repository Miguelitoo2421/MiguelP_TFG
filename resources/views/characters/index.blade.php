...
<x-app-layout>
    ...
    <x-wrapper-views x-data>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            @foreach($characters as $character)
              <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap">{{ $character->play?->name }}</td>
      ...
      <x-modal name="create-character" maxWidth="md" focusable>
        <form action="{{ route('characters.store') }}"
              method="POST"
          ...
          <div class="flex justify-end space-x-2 pt-4 border-t">
            <x-secondary-button type="button"
                                @click="$dispatch('close-modal','create-character')">
              {{ __('Cancel') }}
            </x-secondary-button>
      ...
      {{-- ——— Modales: Edit Character ——— --}}
      @foreach($characters as $character)
        <x-modal name="edit-character-{{ $character->id }}" maxWidth="md" focusable>
          <form action="{{ route('characters.update', $character) }}"
                method="POST"
                enctype="multipart/form-data"
                class="space-y-6 p-6">
            @csrf
            ...
            <div>
              ...
              <x-text-input id="name-{{ $character->id }}"
                            name="name"
                            type="text"
                            class="mt-1 block w-full"
            ...
            <div>
              ...
              <select id="play_id-{{ $character->id }}"
                      name="play_id"
                      class="mt-1 block w-full border-gray-300 rounded">
                <option value="">{{ __('— None —') }}</option>
                @foreach($plays as $id => $name)
                  <option value="{{ $id }}"
                          {{ old('play_id', $character->play_id) == $id ? 'selected' : '' }}>
                    {{ $name }}
                  </option>
                @endforeach
              </select>
            ...
            <div>
              ...
              <textarea id="notes-{{ $character->id }}"
                        name="notes"
                        class="mt-1 block w-full border-gray-300 rounded">{{ old('notes', $character->notes) }}</textarea>
              <x-input-error :messages="$errors->get('notes')" class="mt-2" />
            </div>
  
            <!-- Image -->
            <div>
              <x-input-label for="image-{{ $character->id }}" :value="__('Image')" />
              <input id="image-{{ $character->id }}"
                     name="image"
                     type="file"
                     class="mt-1 block w-full" />
              @if($character->image)
                <img src="{{ Storage::url($character->image) }}"
            ...
            <div class="flex justify-end space-x-2 pt-4 border-t">
              <x-secondary-button type="button"
                                  @click="$dispatch('close-modal','edit-character-{{ $character->id }}')">
                {{ __('Cancel') }}
              </x-secondary-button>
        ...
        <x-modal name="confirm-delete-character-{{ $character->id }}" focusable>
          <form action="{{ route('characters.destroy', $character) }}"
            ...
            <div class="mt-4 flex justify-end space-x-2">
              <x-secondary-button
                  @click="$dispatch('close-modal','confirm-delete-character-{{ $character->id }}')">
                {{ __('Cancel') }}
              </x-secondary-button>
