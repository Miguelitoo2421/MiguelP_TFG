...
<x-app-layout >
    ...
    <x-wrapper-views x-data>
      {{-- Slot actions: aquí va el botón de “Register New User” --}}
      <x-slot name="actions">
        ...
        <x-modal name="edit-user-{{ $user->id }}" maxWidth="md" focusable>
          <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-6 p-6">
            ...
            <div>
              ...
              <x-text-input id="name-{{ $user->id }}" name="name" type="text"
            ...
            <div>
              ...
              <x-text-input id="email-{{ $user->id }}" name="email" type="email"
            ...
            <div>
              ...
              <select name="roles[]" class="mt-1 block w-full border-gray-300 rounded" required>
      ...
      <x-modal name="create-user" maxWidth="md" focusable>
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-6 p-6">
          ...
          <div>
            <x-hidden-password name="password" id="password" label="{{ __('Password') }}"
              autocomplete="new-password" />
          </div>
  
          <div>
            <x-hidden-password name="password_confirmation" id="password_confirmation"
          ...
          <div class="flex justify-end space-x-2 pt-4 border-t">
            <x-secondary-button type="button" @click="$dispatch('close-modal','create-user')">
              {{ __('Cancel') }}
            </x-secondary-button>
